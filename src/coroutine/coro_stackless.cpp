...
BOOL CGlobalStacklessMgr::init(int32_t nServerAddr, int32_t nMgrShmType, 
			int32_t nBeginShmType, int32_t nEndShmType, int32_t nCoroCount, BOOL bResume)
{
    int32_t nRetCode = 0;
    ...
    nRetCode = m_MgrData.init(nMgrShmType, bResume);
    ...
    if (!bResume)
    {
        ...
        for (std::vector<CORO_MGR_DATA>::iterator it = m_CoroStacklessMgrList.begin(); it != m_CoroStacklessMgrList.end(); it++)
        {
            ...
            strxcpy(pMgrData->stShmTypeInfo[nInfoCount].szMgrName, it->szMgrName, MAX_MGR_NAME_LEN);
            pMgrData->stShmTypeInfo[nInfoCount].nShmType = nBeginShmType + nInfoCount;
            LOG_PROCESS_ERROR(pMgrData->stShmTypeInfo[nInfoCount].nShmType <= nEndShmType);
    ...
    for (std::vector<CORO_MGR_DATA>::iterator it = m_CoroStacklessMgrList.begin(); it != m_CoroStacklessMgrList.end(); it++)
    {
        ...
        DBG("[CORO]: init coro mgr %s with shm_type %d count %d", it->szMgrName, nShmType, nCoroCount);

        nRetCode = pCoroMgr->init(nShmType, nShmType - nBeginShmType, nCoroCount, bResume);
        LOG_PROCESS_ERROR(nRetCode);
    }
