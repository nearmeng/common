...
BOOL CShmObjectPool<T, N>::init(int32_t shm_type, int32_t unit_count, BOOL is_resume)
{
	...
	LOG_PROCESS_ERROR_DETAIL(shm_type > 0, "shm_type is %d", shm_type);
	LOG_PROCESS_ERROR_DETAIL(shm_type < MAX_SHM_TYPE_COUNT, "shm type is %d", shm_type);
...
inline SHM_POOL* CShmMgr::get_pool(int32_t shm_type)
{
	...
	LOG_PROCESS_ERROR(shm_type >= 0 && shm_type < MAX_SHM_TYPE_COUNT);
