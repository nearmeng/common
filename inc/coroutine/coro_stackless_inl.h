...
template<class T>
BOOL CCoroStacklessMgr<T>::init(int32_t nShmType, int32_t nMgrIndex, BOOL nInitCoroCount, BOOL bResume)
{
    int32_t nRetCode = 0;

    m_nMgrIndex = nMgrIndex;
...
void CCoroStacklessMgr<T>::mainloop(void)
{
    ...
    while (m_RunLinkHead.qwHeadObjMid != 0)
    {
        ...
        if (CTimeMgr::instance().get_time_sec() > pCoro->get_yield_time() + CORO_WAIT_INTERVAL / 1000)
        {
